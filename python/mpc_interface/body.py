...
class Formulation:
    def __init__(self):
        ...
        self.domain_ID = {"optim_ID": self.optim_ID, "given_ID": self.given_ID}
    ...
    def update_qp_IDs(self):
        ...
        optim_ranges = [
            range(sum(self.optim_sizes[:i]), sum(self.optim_sizes[: i + 1]))
            for i in range(len(self.optim_sizes))
        ]

        given_ranges = [
            range(sum(self.given_sizes[:i]), sum(self.given_sizes[: i + 1]))
    ...
    def make_preview_matrices(self):
        ...
        for variable in self.definitions.keys():
            if variable in self.of.keys():
                self.PM.update({variable: self.get_matrices_from_dynamics(variable)})
            else:
    ...
    def generate_qp_cost(self, cost, given):
        ...
        for i, axis in enumerate(cost.axes):
            ...
            if cost.L:
                ...
                vMg = self.PM[var][0][schedule]
                vMo = self.PM[var][1][schedule]

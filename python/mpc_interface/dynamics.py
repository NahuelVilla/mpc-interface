...
class DomainVariable:
    def __init__(
        ...
        if len(names) != len(sizes):
            raise IndexError(
                "'names' and 'sizes' must have the same amount " + "of elements."
            )
    ...
    def define_output(self, name, combination, time_variant=False, how_to_update=None):
        """
            ...
            self.definitions.update(
                {
                    name
                    + axis: LineCombo(
                        comb, time_variant=time_variant, how_to_update=how_to_update
                    )
                }
            )
...
class ExtendedSystem:
    ...
    def from_control_system(cls, control_system, state_vector_name, horizon_length):
        ...
        self = cls(
            control_system.input_names,
            control_system.state_names,
            state_vector_name,
            S,
            U,
            control_system.axes,
            control_system.time_variant,
    ...
    def identify_domain(self, input_names, state_names):
        ...
        self.state_ID = {}
        for axis in self.axes:
            self.domain_ID.update({name + axis: ID for name, ID in domain_ID.items()})
            self.state_ID.update({name + axis: ID for name, ID in state_ID.items()})

    def set_sizes(self):
        state_sizes = {
            state: self.matrices[-1].shape[0] for state, ID in self.state_ID.items()
        }
        self.domain = {
            base: self.matrices[ID].shape[1] for base, ID in self.domain_ID.items()
        }
    ...
    def make_definitions(self):
        ...
        for variable, sID in self.state_ID.items():
            ...
            self.definitions[variable]._coefficients = (len(self.matrices) - 1) * [
                "U"
            ] + ["S"]
...
class ControlSystem:
    ...
    def from_name(
        ...
        **kargs
        ...
        self = cls(
            input_names, state_names, A, B, axes, time_variant, how_to_update_matrices
        )
